...
try:
    ...
    # 2. Pull code mới từ Github (dùng git reset --hard để đồng bộ sạch sẽ)
    run_cmd("cd /opt/cenar-store && git fetch --all")
    run_cmd("cd /opt/cenar-store && git reset --hard origin/main")
