...
const STORE1_TERMS_CHAN_ID = '1514605940982218763';
...
client.once('ready', async () => {
  ...
  try {
    ...
    if (announceCh) {
      ...
      const sent = await announceCh.send({ content: announcementText });
  ...
  try {
    ...
    if (termsCh) {
      ...
      const sent = await termsCh.send({ content: termsText });
