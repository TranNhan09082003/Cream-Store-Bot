import { SlashCommandBuilder, PermissionFlagsBits, EmbedBuilder, ModalBuilder, ActionRowBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
import {
...
export const data = new SlashCommandBuilder()
  ...
  .setDescription('Quản lý gia hạn Nitro / Spotify / YouTube cho chủ shop')
  ...
  .addSubcommand(s =>
    s.setName('list').setDescription('Xem danh sách subscriptions')
      .addStringOption(o => o.setName('loai').setDescription('Lọc theo loại').setRequired(false)
        .addChoices({ name: 'Nitro', value: 'nitro' }, { name: 'Spotify Family', value: 'spotify_family' }, { name: 'YouTube', value: 'youtube' }))
  )
  .addSubcommand(s =>
...
function buildNitroModal() {
  ...
  modal.addComponents(
    ...
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('purchase_date').setLabel('Ngày mua (DD/MM/YYYY hoặc để trống = hôm nay)').setPlaceholder('06/05/2026').setStyle(TextInputStyle.Short).setRequired(false)),
  );
  return modal;
}
...
function buildYoutubeModal() {
  ...
  modal.addComponents(
    ...
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('type').setLabel('Loại: "thang" (hàng tháng) hoặc "full" (1 lần)').setPlaceholder('thang').setStyle(TextInputStyle.Short).setRequired(true).setValue('thang')),
...
function buildListEmbed(subs, filterType) {
  ...
  for (const [type, items] of Object.entries(grouped)) {
    ...
    for (const s of items.slice(0, 15)) {
      ...
      const extra = s.spotify_family_name ? ` 🏠 ${s.spotify_family_name} (${s.spotify_slots_used}/5)` : '';
...
export { buildNitroModal, buildSpotifyModal, buildYoutubeModal };
