...
function getOutstandingOrdersStmt(){return db.prepare(`SELECT * FROM orders WHERE guild_id=? AND status IN ('PENDING_PAYMENT','PROCESSING','WARRANTY_OPEN') AND (? IS NULL OR customer_id=?) ORDER BY priority_rank DESC, created_at ASC LIMIT ?`);}
...
function generateUniqueOrderCode(){while(true){const c=`CR_${randomDigits(6)}`; if(!orderCodeExistsStmt().get(c)) return c;}}
...
export function createOrder({ guildId, ticketId, ticketChannelId, customerId, productName, quantity, note, totalAmount = 0, durationMonths = config.defaultOrderDurationMonths, orderLogChannelId, createdById }) {
  ...
  const payosOrderCode = Number(orderCode.replace('CR_', ''));
...
export const getOutstandingOrders = (guildId, customerId=null, limit=20) => getOutstandingOrdersStmt().all(guildId, customerId, customerId, limit);
